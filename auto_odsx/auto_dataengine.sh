...
usage() {
  ...
  $(basename $0) -f gilboafull                      # Run gilboafull then gilboaupdate with is-2weeks-before=true
  $(basename $0) -f gilboaupdate                    # Run gilboaupdate with is-2weeks-before=false
...
gilboafull_feeder() {
  ...
  local output=$( /gigashare/current/mssql/scripts/load_Portal_Calendary_View_V2.sh $space_server_ip $port 2>/dev/null )
  echo "Start action sent to table ${table}" >> $_LOGFILE
  [[ "$(echo $output | grep -o '"OK"')" != '"OK"' ]] && return 1
...
do_gilboafull_feeder() {
  ...
  _TABLE="Portal_Calendary_View_V2"
  _TABLE_NAME="dbo.Portal_Calendary_View_V2"
  _TRUE="true"
  gilboafull_feeder
  ...
  if [[ $gilboafull_feeder_exit_code -eq 0 ]] ; then
...
gilboaupdate_feeder() {
  ...
  _TABLE="Portal_Calendary_Changes_View_V2"
  _TABLE_NAME="dbo.Portal_Calendary_View_V2"
  echo -e "_DB_FILE=$_DB_FILE _TABLE=$_TABLE _TABLE_NAME=$_TABLE_NAME "
  local table="${_TABLE,,}"
  local table_name="${_TABLE_NAME}"
  ...
  ## do start action and check for "OK"
  echo -e "curl -XPOST \"http://${space_server_ip}:${port}/table-feed/start?table-name=${table_name}&base-column=v_timestamp&exclude-columns=${exclude_columns}&is-2weeks-before=${_TRUE}\"" >> $_LOGFILE
  echo -e "_TRUE=${_TRUE}"
  exclude_columns="code_last_action,change_lecturer,change_room,change_date,change_time"
  pk_columns=""
  echo table=$table table_name=$table_name space_server_name=$space_server_name port=$port space_server_ip=$space_server_ip 
  local output=$(curl -s -XPOST "http://${space_server_ip}:${port}/table-feed/start?table-name=${table_name}&base-column=v_timestamp&exclude-columns=${exclude_columns}&is-2weeks-before=${_TRUE}")
