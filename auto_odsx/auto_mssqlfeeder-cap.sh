...
_LOG="/dbagigalogs/mssql-cap.log"
_NUM_OF_TABLES=0      # No. of db2feeder tables
_TABLES=""            # List of tables
_ALL_MANAGERS=( $(/dbagiga/utils/host-yaml.sh -m) )       # List ODS managers
...
if [[ "${ENV_NAME}" == "STG" ]] ; then
   JOTBMF01_TN_MATI=5000000
  JOTBMF11_MATI_ISKY=100000
    JOTBMFM1_TN_MATI=100000
    JOTBPM01_TN_MATH=100000
   JOTBPM03_EXT_MATH=100000
elif [[ "${ENV_NAME}" == "DEV" || "${ENV_NAME}" == "GRG" || "${ENV_NAME}" == "DR" || "${ENV_NAME}" == "PRD" ]] ; then
    JOTBMF01_TN_MATI=1000000
  JOTBMF11_MATI_ISKY=100000
    JOTBMFM1_TN_MATI=100000
    JOTBPM01_TN_MATH=100000
   JOTBPM03_EXT_MATH=100000
